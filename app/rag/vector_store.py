...
class VectorStoreManager:
    ...
    def _load_or_create_store(self) -> Optional[FAISS]:
        ...
        try:
            if os.path.exists(self.persist_directory) and os.listdir(
                ...
                logger.info(
                    f"Loading existing FAISS index from {self.persist_directory}..."
                )
                ...
                logger.info(
                    "No existing index found. Returning None to indicate empty store."
                )
                return None
    ...
    def similarity_search(
        self, query: str, k: int = 5, score_threshold: float = 0.25
    ) -> List[Document]:
        """
        Performs a similarity search on the vector store.

        Scores are cosine similarities derived from FAISS squared-L2
        distances (embeddings are L2-normalized, so cosine = 1 - dist / 2),
        clamped to [0, 1] and stored in ``metadata["relevance_score"]``.
        """
        logger.info(f"Performing similarity search for query: '{query}' (k={k})")
        with self._lock:
            if self.vector_store is None:
                logger.warning(
                    "Vector store is empty or uninitialized. Returning no results."
                )
                return []
            ...
            try:
                ...
                logger.warning(
                    "Relevance scores not supported by current FAISS "
                    "index configuration. Using standard search."
                )
    ...
    def delete_documents(self, doc_ids: List[str]) -> None:
        ...
        with self._lock:
            if self.vector_store is None:
                logger.warning(
                    "Cannot delete from an empty/uninitialized vector store."
                )
                return
    ...
    def get_document_count(self) -> int:
        ...
        with self._lock:
            ...
            try:
                count = int(self.vector_store.index.ntotal)
                return count
            except Exception as e:
