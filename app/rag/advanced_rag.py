...
1. Agentic RAG: Corrective RAG (CRAG) + Self-Reflective RAG (Self-RAG)
   + Multi-Hop Reasoning
2. Vectorless RAG: Entity-Relationship Knowledge Graph + Inverted BM25
   Index + Document Tree Routing
3. Hierarchical Parent-Child RAG: Precision child search with contextual
   parent retrieval
...
class AdaptiveChunker:
    """
    Selects and executes optimal chunking strategies tailored to document
    ...
    - Large docs (> 20,000 chars): Hierarchical Parent-Child chunking +
    ...
    def chunk_document(doc: Document) -> Tuple[List[Document], List[Document]]:
        ...
        For large documents, parent contains rich context and child
        contains targeted snippets.
        """
...
class VectorlessRAG:
    ...
    def __init__(self) -> None:
        ...
        # Knowledge Graph: entity -> set of (relation, target, doc_idx)
    ...
    def extract_entities(self, text: str) -> List[str]:
        ...
        entities = list(
            set([e.lower() for e in acronyms + capitalized + compounds if len(e) > 2])
        )
        return entities
    ...
    def fit(self, documents: List[Document]) -> None:
        ...
        logger.info(
            f"VectorlessRAG index built: {len(documents)} docs, "
            f"{len(self.graph)} entities, {len(self.section_tree)} sections."
        )
    ...
    def search(self, query: str, top_k: int = 5) -> List[Document]:
        ...
        for doc in bm25_hits:
            ...
            orig_idx = next(
                (
                    ...
                    if d.page_content.startswith(content_prefix)
                ),
                None,
            )
        ...
        for doc_idx, bonus in sorted(
            graph_bonus.items(), key=lambda x: x[1], reverse=True
        )[:3]:
...
class AgenticRAG:
    ...
    - Self-RAG: Evaluates assertion grounding [IS_SUPPORTED] to
      eliminate hallucinations.
    - Multi-Hop Decomposer: Executes multi-step retrieval hops for compound queries.
    """

    def __init__(
        self, hybrid_retriever: Any, vectorless_rag: Optional[VectorlessRAG] = None
    ):
    ...
    def evaluate_retrieval_quality(
        ...
        Returns evaluation rating: 'CORRECT' (high confidence),
        'AMBIGUOUS' (medium), or 'INCORRECT' (low).
        """
        if not documents:
            return {
                "rating": "INCORRECT",
        ...
        for doc in documents:
            ...
            if (
                overlap_ratio >= 0.45
                or doc.metadata.get("relevance_score", 0.0) >= 0.80
            ):
        ...
        confidence = (num_correct * 0.7 + len(ambiguous_docs) * 0.3) / max(
            len(documents), 1
        )
    ...
    def self_rag_grounding_check(
        self, response_text: str, context_docs: List[Document]
    ) -> Dict[str, Any]:
        """
        ...
        for sentence in sentences:
            s_words = [
                w
                for w in re.findall(r"\b\w{4,}\b", sentence.lower())
                if w not in {"with", "that", "this", "from", "have"}
            ]
            if not s_words:
                supported += 1
                continue
    ...
    def multi_hop_retrieve(self, query: str, top_k: int = 6) -> List[Document]:
        ...
        Hop 2: Uses Hop 1 findings to retrieve deeper trade-offs, edge
        ...
        if top_entities:
            hop2_query = (
                f"{query} {' '.join(top_entities)} benchmarks limitations performance"
            )
            hop2_docs = self.hybrid_retriever.retrieve(
                hop2_query, mode="multi_query", top_k=top_k // 2
            )
        else:
            hop2_docs = []
    ...
    def execute_agentic_rag(
        ...
        # On AMBIGUOUS/INCORRECT CRAG rating, supplement with the
        # Vectorless Knowledge Graph
        if eval_result["rating"] in ["AMBIGUOUS", "INCORRECT"] and self.vectorless_rag:
            logger.info(
                f"CRAG rating was {eval_result['rating']}. Triggering "
                f"Vectorless Knowledge Graph expansion..."
            )
